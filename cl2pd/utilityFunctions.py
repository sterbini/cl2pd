...
def getBunchLumiFromMassiDF(massiDF):
    """
    The function takes as input the massDF generated by the importData.massiFile2pd
    and groups the "Bunch" column in order to provide a matrix-like data format with
    bunch numbers being the columns and luminosity (column name: "Luminosity Hz/ub")
    being the cell values.
    The datetime index is properly kept.
    """
    retDF = pd.DataFrame()
    for ibunch in massiDF['Bunch'].unique():
        retDF[ibunch] = massiDF['Luminosity [Hz/ub]'].groupby(massiDF['Bunch']).get_group(ibunch)
    retDF.index = massiDF['Luminosity [Hz/ub]'].groupby(massiDF['Bunch']).get_group(massiDF['Bunch'].unique()[0]).index
    return retDF
